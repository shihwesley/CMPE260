...
from car import MAX_SPEED, VISION_B, VISION_F, VISION_W
from config import INPUT_VIEW_OFFSET_X, INPUT_VIEW_OFFSET_Y, ROAD_VIEW_OFFSET
...
def draw_received_info(surface, car, position):
    if not config.VISUALENABLED:
        return

    # Example: Display the speed and lane of the car
    info_text = f"Speed: {car.speed} km/h, Lane: {car.lane}"
    info_surface = font_28.render(info_text, True, black)
    print("Blitting info_surface at position:", position)
    surface.blit(info_surface, position)

def print_received_info(car, all_cars):
    # Extract relevant information from the car object
    car_info = car.broadcast_info(all_cars)

    # Format and print the information
    print(f"Car ID: {car_info['id']}, Position: ({car_info['x']}, {car_info['y']}), Speed: {car_info['speed']} km/h, lane: {car_info['lane']} lane, Intended Action: {car_info['intended_action']}")
...
def draw_dashed_line(surf, color, start_pos, end_pos, width=1, dash_length=10):
    ...
    if displacement.x != 0:
        slope = displacement.y / displacement.x
    else:
        slope = float('inf') if displacement.y >= 0 else float('-inf')
    loop = length / dash_length

    for index in range(0, int(loop), 2):
        start = origin + Point((slope * index * dash_length, 0))
        end = origin + Point((slope * (index + 1) * dash_length, 0))
        if not math.isnan(start.x) and not math.isnan(start.y) and not math.isinf(start.x) and not math.isinf(start.y):
...
def draw_dashed_line_delay(surf, color, start_pos, end_pos, width=1, dash_length=10, delay=0):
    ...
    origin = Point((origin.x + delay * 10, origin.y + slope * delay * 10))
...
def identify_free_lane(cars):
    lanes = [[n for n in range(1, 7)] for _ in range(2)]
    for car in cars:
        if -170 <= car.y <= 0:
